...
class XRASPINNSolver:
    ...
    def set_history_field(self, x_domain, d_prev_field):
        """将全域历史场切分并存储"""
        x_sing, x_far, mask_sing, mask_far = self.partition_points(x_domain)

        self.d_prev_1 = d_prev_field[mask_sing].detach().clone().to(self.device)
        self.d_prev_2 = d_prev_field[mask_far].detach().clone().to(self.device)
        print(f"  [X-RAS] History field set. Ω_sing max={self.d_prev_1.max():.3f}, Ω_far max={self.d_prev_2.max():.3f}")
