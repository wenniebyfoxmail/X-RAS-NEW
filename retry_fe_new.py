...
def run_sent_phasefield_vertical(
    ...
    nx_fe = int(cfg.get("nx_fe", 640))
    ny_fe = int(cfg.get("ny_fe", 640))
    ...
    # === 损伤能量 (使用历史场 H) ===
    # 这里只用 H 驱动拉伸部分，压缩部分 psi_minus 不驱动裂纹
    damage_energy = (g_d * H + Gc_val * (d / ell_val + ell_val * ufl.inner(ufl.grad(d), ufl.grad(d)))) * dx

    # 损伤的变分方程：F_d(d; H) = 0
    F_d = ufl.derivative(damage_energy, d, v_d)
    J_d = ufl.derivative(F_d, d, d_trial)
